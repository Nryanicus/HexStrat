...
{
    if (cache.has(start.toString()+goal.toString()))
        return cache.get(start.toString()+goal.toString());
    ...
    while (open.size > 0 || derp != 10)
    {
        ...
        if (current.toString() == goal.toString())
        {
            ...
            cache.set(start.toString()+goal.toString(), path);
            return path;
        }
